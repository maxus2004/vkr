...
Цепь сброса включает подтягивающий резистор R1, соединяющий вывод RESET с линией +3.3 В, и кнопку SB1, замыкающую этот вывод на общую землю; при нажатии кнопки микроконтроллер переходит в состояние сброса. Также для сглаживания дребезга при нажатии на кнопку сброса стоит конденсатор C5. Для ручного перехода в режим прошивки есть кнопка SB2, она нужна для перехода в режим прошивки, если программно это сделать не удастся из за ошибки программирования. Сигнал режима программирования стянут к земле резистором К2, так как режим программивания включается при высоком сигнле на входе BOOT0, а по умолчанию микроконтроллер должен работать в обычном режиме.
...
При вращении вала энкодер генерирует на своих выходах последовательность прямоугольных импульсов.
Дорожки в механических энкодерах или датчики холла в магнитных смещены относительно друг друга на 1/4 периода (90 градусов). Это смещение создает фазовый сдвиг между сигналами каналов A и B, который является критерием определения направления вращения.
При вращении вперед сигнал канала A опережает канал B на 90°.
При вращении назад сигнал канала B опережает канал A на 90°.
...
=== Разработка структурной схемы системы автопилотирования
...
typedef struct {
    ...
    int16_t  steer_angle; // угол поворота, десятые доли градуса
...
typedef struct {
    ...
    int16_t  speed_fl;         // скорость переднего левого мотора, мм/с
    int16_t  speed_fr;         // скорость переднего правого мотора, мм/с
    int16_t  speed_rl;         // скорость заднего левого мотора, мм/с
    int16_t  speed_rr;         // скорость заднего правого мотора, мм/с
    int16_t  uwb_distances[4]; // расстояния до меток DWM1000, мм
    uint8_t  lidar_data[LIDAR_MAX_POINTS];  // данные сканирования LD06
